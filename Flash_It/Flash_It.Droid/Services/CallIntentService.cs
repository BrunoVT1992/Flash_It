...
namespace Flash_It.Droid.Services
{
    ...
    public class CallIntentService : IntentService
    {
        ...
        {
			if (CallPreferences.Enabled && !_ringer.FlashInProgress && _battery.CheckBatteryLevelAllowed() && _ringer.CheckIfProfileIsAllowed())
            {
				_ringer.FlashInProgress = true;

                do
                {
					_camera.Flash(CallPreferences.OnTime);
					await Task.Delay(CallPreferences.OffTime);
				} while (_ringer.CallState == CallState.Ringing);

                _ringer.FlashInProgress = false;
            }
        }
    }
}